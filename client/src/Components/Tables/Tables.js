...
const Tables = () => {
  ...
  const fetchTablename = async () => {
    try {
      const response = await fetch("http://localhost:5000/gettablename");
      const data = await response.json();
      console.log("Fetched table names jalabulajangu:", data);
      setTablename(data);
    } catch (error) {
  ...
  const fetchTableData = async (tableName) => {
    try {
      const response = await fetch("http://localhost:5000/tablecategorieswithvalue", {
        method: "POST",
        headers: {
